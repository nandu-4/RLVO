...
## 1 · Stateless deployment (fastest path)

Verification only. Nothing stored; review, analytics, batch and audit are unavailable and the UI
says so per feature.
...
Verify: `curl https://<your-app>/api/health` → `"status": "healthy"`, persistence mode `stateless`.

---

## 2 · Workspace deployment (full platform)
...
```bash
# Apply ALL FOUR migrations, in filename order. The second is not optional:
...
| `SUPABASE_URL` | No | Presence switches the API into workspace mode |
| `SUPABASE_SERVICE_ROLE_KEY` | No | Required alongside `SUPABASE_URL` |
| `DEFAULT_RETENTION_DAYS` | No | Default 30. Per-workspace override in Admin |
...
- [ ] Retention policy set to your requirement (Admin → Workspace)
- [ ] Understood: **there is no authentication**. The workspace token is a bearer secret. Do not
      process regulated personal or health data on this deployment
